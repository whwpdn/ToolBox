...
**Phase 0~3 완료, Phase 4 진행 중** — 코어 프레임워크와 도구 17개가 동작하고,
Docker 이미지가 CI에서 자동 빌드·검증되어 GHCR에 올라간다.

| 항목         | 상태                                                      |
| ------------ | --------------------------------------------------------- |
| 도구         | 17개 (8개 카테고리 전부 최소 1개)                         |
| 단위 테스트  | 355개 통과 (계산 로직 전량)                               |
| 초기 번들    | 51.6 kB (gzip) — 목표 200 kB 이내                         |
| 도구별 chunk | 0.8 ~ 3.4 kB (gzip), 진입 시에만 로드                     |
| CI           | 타입검사 · 린트 · 테스트 · 빌드 → Docker 이미지 GHCR 푸시 |

구현된 도구: 계산기, **공학용 계산기**, 퍼센트 계산기, 길이/무게/면적/온도 단위 변환,
이자 계산기, **예금·적금 계산기**, 대출 상환 계산기, 대출 한도 계산기, **부가세 계산기**,
날짜 계산기, 만 나이 계산기, BMI 계산기, 수학 공식 모음, 글자수 세기

Phase 4(확장 도구) 진행 중 — 다음은 연봉 실수령액, 부피·속도 단위 변환, D-Day.
목록은 [03-tool-catalog.md](docs/03-tool-catalog.md) 참고.
