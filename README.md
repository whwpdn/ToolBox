...
**Phase 0~3 완료, Phase 4 진행 중** — 코어 프레임워크와 도구 22개가 동작하고,
Docker 이미지가 CI에서 자동 빌드·검증되어 GHCR에 올라간다.

| 항목         | 상태                                                      |
| ------------ | --------------------------------------------------------- |
| 도구         | 22개 (8개 카테고리 전부 최소 1개)                         |
| 단위 테스트  | 470개 통과 (계산 로직 전량)                               |
| 초기 번들    | 53.3 kB (gzip) — 목표 200 kB 이내                         |
| 도구별 chunk | 0.8 ~ 4.3 kB (gzip), 진입 시에만 로드                     |
| CI           | 타입검사 · 린트 · 테스트 · 빌드 → Docker 이미지 GHCR 푸시 |

| 카테고리      | 도구                                                                      |
| ------------- | ------------------------------------------------------------------------- |
| 계산기        | 계산기, 공학용 계산기, 퍼센트 계산기                                      |
| 단위 변환     | 길이, 무게, 면적(평↔㎡), 온도                                             |
| 금융 · 부동산 | 이자, 예금·적금, 대출 상환, 대출 한도, 부가세, **취득세**, **중개수수료** |
| 수학          | 공식 모음                                                                 |
| 날짜 · 시간   | 날짜 계산, 만 나이, **시간 계산(시분초)**, **타임스탬프·UTC**             |
| 건강 · 생활   | BMI, **가족 호칭(촌수)**                                                  |
| 텍스트        | 글자수 세기                                                               |

Phase 4(확장 도구) 진행 중 — 다음은 연봉 실수령액, 부피·속도 단위 변환, D-Day.
양도소득세는 범위 확정 후 진행합니다([사유](docs/04-development-plan.md)).
전체 목록은 [03-tool-catalog.md](docs/03-tool-catalog.md) 참고.
...
npm run smoke          # 브라우저 스모크 테스트 (preview 실행 중일 때)
```

### 포트 변경

```bash
npm run dev -- --port 3000        # 일회성 (-- 가 있어야 vite로 전달됨)
PORT=3000 npm run dev             # 환경변수
PREVIEW_PORT=8080 npm run preview # preview 서버 포트

npm run dev -- --host             # 다른 기기(폰 등)에서 접속 허용
```

기본값은 `vite.config.ts` 의 `server.port` / `preview.port` 에 있습니다.
포트가 이미 사용 중이면 Vite가 조용히 다음 번호로 옮겨가므로, 주소가 예상과 다르면
터미널에 찍힌 `Local:` 줄을 확인하세요.
...
`src/tools/salary/` 에 `meta.ts` · `View.vue` · `logic.ts` · `logic.spec.ts` 골격이 생성되고,
