...
3. [x] `vat` 부가세 계산기
4. [x] `kinship` 가족 호칭 계산기 (촌수 포함)
5. [x] `time-calc` 시분초 계산기 · `timestamp` 타임스탬프·UTC 변환
6. [x] `acquisition-tax` 취득세 · `brokerage-fee` 중개수수료
7. [ ] `salary` 연봉 실수령액
8. [ ] `unit-volume` / `unit-speed` / `unit-data`
9. [ ] `dday` · `workdays` · `timezone`
10. [ ] `bmr` · `electricity` · `fuel-cost`
11. [ ] `json-format` · `base64` · `hash` · `color-convert`
12. [ ] `gcd-lcm` · `radix` · `statistics`
13. [ ] `lotto`

### 보류: 양도소득세 계산기

요청은 받았으나 구현하지 않았다. 다른 도구와 위험의 성격이 다르다.

취득세·중개보수는 **입력 몇 개로 답이 정해지는** 계산이라 상수만 정확하면 맞는 값이 나온다.
반면 양도소득세는 장기보유특별공제(보유·거주 기간별 표), 1세대1주택 비과세(12억 기준),
다주택 중과, 필요경비 인정 범위, 기본공제, 누진세율이 서로 맞물린다.
입력을 단순화하면 **틀린 값이 그럴듯하게 나오고**, 그 숫자로 수천만 원짜리 판단을 하게 된다.
'참고용'이라고 적어도 이 위험은 줄지 않는다.

진행하려면 범위를 먼저 정해야 한다.

- 대상: 1세대1주택만? 다주택 포함? 분양권·입주권은?
- 장기보유특별공제: 표 전체를 넣을지, 보유 기간만 볼지
- 비과세 판정을 도구가 할지, 사용자가 고르게 할지

가장 안전한 1차 범위는 **1세대1주택 양도차익 계산기**다.
세액 대신 양도차익과 과세 대상 여부만 알려주고, 세액은 홈택스 모의계산으로 넘긴다.

### 호칭 계산기 설계 메모

호칭은 규칙으로 유도되지 않는다. '아버지의 형'이 큰아버지인 것은 관습이지 계산이 아니다.
그래서 **사전(경로 → 호칭)** 방식으로 담고, 촌수만 규칙으로 계산해 사전에 없는 조합에서도
의미 있는 답을 주도록 했다.

- 촌수: 부모·자식 1촌, 형제 2촌을 더한다. 배우자는 무촌이라 혈족 촌수를 매기지 않는다
- 화자 성별은 두 곳에 영향을 준다 — 선택지 라벨(형/오빠)과 결과 호칭(형수님/새언니)
- **배우자 제약은 첫 단계에만 적용된다.** 첫 단계의 배우자는 '나의 배우자'지만
  두 번째부터는 앞사람의 배우자다. 여자도 '나의 오빠의 아내'(새언니)를 고를 수 있어야 한다
  (브라우저 검증에서 발견해 고친 버그)
